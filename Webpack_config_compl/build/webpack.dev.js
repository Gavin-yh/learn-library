...
const devConfig = {
    ...
    plugins: [
        new HtmlWebpackPlugin({                         //用于自动在dist生成html，  （npm i html-webpack-plugin -D）
            filename: 'demo1.html',                     //天坑，这里在run dev时不能使用html/home.html    打包的时候可以按照自定义来打包。但是跑服务器不能加层级也不能改名字;
            template: './src/html/demo1.html',                   //模板html，会往模板html注入js
        ...
        new HtmlWebpackPlugin({                         //用于自动在dist生成html，  （npm i html-webpack-plugin -D）
            filename: 'demo2.html',                     //天坑，这里在run dev时不能使用html/home.html    打包的时候可以按照自定义来打包。但是跑服务器不能加层级也不能改名字;
            template: './src/html/demo2.html',                   //模板html，会往模板html注入js
            // hash:true,                               //消除缓存，添加版本号  
            chunks: ["about"],                                
            minify: {
                removeComments: true,                   //去掉注释   可以自己改
